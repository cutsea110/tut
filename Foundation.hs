...
--
-- * Creates the route datatype TutRoute. Every valid URL in your
--   application can be represented as a value of this type.
-- * Creates the associated type:
--       type instance Route Tut = TutRoute
-- * Creates the value resourcesTut which contains information on the
--   resources declared below. This is used in Handler.hs by the call to
--   mkYesodDispatch
--
-- What this function does *not* do is create a YesodSite instance for
-- Tut. Creating that instance requires all of the handler functions
-- for our application to be in scope. However, the handler functions
-- usually require access to the TutRoute datatype. Therefore, we
-- split these actions into two functions and place them in separate files.
mkYesodData "Tut" $(parseRoutesFile "config/routes")

type Form x = Html -> MForm Tut Tut (FormResult x, Widget)
...
instance YesodAuth Tut where
    ...
    getAuthId creds = runDB $ do
        ...
        case x of
            ...
            Nothing -> do
                fmap Just $ insert $ User (credsIdent creds) Nothing 0 ""
...
instance RenderMessage Tut FormMessage where
